...
if (generate_pdf) pdf("plots/t-verteilung-1.pdf", height = 6, width = 8)
plot(x, y, type = "l", xlab = "y", ylab = "density", las = 1, bty = "l")
polygon(c(x0, 1.96, -3), c(y0, 0, 0), col = "grey")
lines(c(1.5, 1.5, 2.5, 2.5, 1.5), c(0, 0.15, 0.15, 0, 0), col = 1, lwd = 3, las = 1, bty = "l")
if (generate_pdf) dev.off()

# Plot 2: Zoomed-in version of the standard normal distribution
if (generate_pdf) pdf("plots/t-verteilung-2.pdf", height = 6, width = 8)
plot(x, y, type = "l", xlim = c(1.5, 2.5), ylim = c(0, 0.15), xlab = "y", ylab = "density", las = 1, bty = "l")
polygon(c(x0, 1.96, -3), c(y0, 0, 0), col = "grey")
abline(v = 1.96, col = 1, lwd = 3)
if (generate_pdf) dev.off()

# Plot 3: Visualization of small changes in the critical value
if (generate_pdf) pdf("plots/t-verteilung-3.pdf", height = 6, width = 8)
plot(x, y, type = "l", xlim = c(1.5, 2.5), ylim = c(0, 0.15), xlab = "y", ylab = "density", las = 1, bty = "l")
abline(v = 1.96, col = 1, lwd = 3)
delta <- 0.2
...
if (generate_pdf) pdf("plots/t-verteilung-4.pdf", height = 6, width = 8)
yy <- pnorm(x)
yy0 <- pnorm(x0)
plot(x, yy, type = "l", xlim = c(1.5, 2.5), ylim = c(0.9, 1), xlab = "y", ylab = "density", las = 1, bty = "l")
lines(c(1.96, 1.96, 0), c(0, 0.975, 0.975), col = 1, lwd = 3, las = 1, bty = "l")
if (generate_pdf) dev.off()

if (generate_pdf) pdf("plots/t-verteilung-6.pdf", height = 6, width = 8)
plot(x, y, type = "l", ylab = "density", xlab = "y", lwd = 2, las = 1, bty = "l")
lines(x, dt(x, df = 1), col = 2, lwd = 2, lty = 2, las = 1, bty = "l")
lines(x, dt(x, df = 10), col = 3, lwd = 2, lty = 3, las = 1, bty = "l")
legend(-3, 0.3, c("normal", "t(1)", "t(10)"), lty = c(1, 2, 3), col = c(1, 2, 3))
if (generate_pdf) dev.off()

# Pivot-based confidence intervals
if (generate_pdf) pdf("plots/KI-pivot-1.pdf", height = 4, width = 6)
xx <- seq(-2.5, 2.5, by = 0.001)
sd <- 0.1
m <- 0.45
plot(xx, dnorm(xx, mean = m, sd = sd), type = "l", 
     ylab = "density", xlab = TeX("\\hat{\\theta}"), las = 1, bty = "l")
if (generate_pdf) dev.off()

if (generate_pdf) pdf("plots/KI-pivot-3.pdf", height = 4, width = 6)
xx <- seq(-2.5, 2.5, by = 0.001)
plot(xx, dnorm(xx, mean = 0, sd = 1), type = "l", 
     ylab = "density", xlab = TeX("( \\hat{\\theta}- \\theta ) / \\sqrt{Var(\\hat{\\theta})}"), las = 1, bty = "l")
