...
if (generate_pdf) pdf("plot-newborns-1.pdf", height = 4, width = 4)
plot(year, male, type = "l", ylim = c(min(female), max(male)), 
     lwd = 2, ylab = "newborns")
lines(year, female, lwd = 2, lty = 2)
```

![](p-value_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 2: Proportion of boys over the years
if (generate_pdf) pdf("plot-newborns-2.pdf", height = 4, width = 4)
plot(year, male /female, type = "l", lwd = 2, ylab = "ratio of boys to girls")
abline(h = 1, lty = 2, lwd = 2)  # ratio of 1 line
```

![](p-value_files/figure-gfm/unnamed-chunk-1-2.png)<!-- -->
...
if (generate_pdf) pdf("plot-newborns-3.pdf", height = 4, width = 4)
xx <- seq(-3, 3, by = 0.001)
plot(xx, dnorm(xx), type = "l", xlab = "z", ylab = "density", lwd = 3)
for (i in 1:length(werte)) {
  abline(v = werte[i], lty = 2)
}
```

![](p-value_files/figure-gfm/unnamed-chunk-1-3.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 4: P-values for pairwise comparisons
if (generate_pdf) pdf("plot-newborns-4.pdf", height = 4, width = 4)
plot(pwerte, ylim = c(0, 1), xlab = "pairwise comparisons", ylab = "p value")
```

![](p-value_files/figure-gfm/unnamed-chunk-1-4.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 5: Distribution of p-values for varying shifts
if (generate_pdf) pdf("plot-p-value.pdf", height = 6, width = 6)
pp <- seq(0, 1, by = 0.001)
plot(pp, pp, type = "l", ylab = "P( p value < p )", xlab = "p", lwd = 3)
c <- 0.5
lines(pp, 1 - pnorm(qnorm(1 - pp) - c), lty = 2, col = "blue", lwd = 3)
c <- 1
lines(pp, 1 - pnorm(qnorm(1 - pp) - c), lty = 3, col = "dark green", lwd = 3)
legend(0.7, 0.35, lty = c(1, 2, 3), 
       c("c=0", "c=0.5", "c=1"), 
       col = c(1, "blue", "dark green"))
```

![](p-value_files/figure-gfm/unnamed-chunk-1-5.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 6: P-value curve for varying boy counts
if (generate_pdf) pdf("plot-newborns-5.pdf", height = 4, width = 4)
boys <- seq(795478 / 2, 408490, by = 100)
plot(boys, 1 - pnorm((boys - 795478 / 2) / (sqrt(795478 * 0.25))), 
     type = "l", ylab = "p value")
```

![](p-value_files/figure-gfm/unnamed-chunk-1-6.png)<!-- -->
