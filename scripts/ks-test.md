...
if (generate_pdf) pdf("KS.pdf", height = 6, width = 6)
...
lines(c(x[Index], x[Index]), c(Fn(x[Index]), pnorm(x[Index])), lwd = 3)
...
lines(x, Fn(x) - pnorm(x))
abline(h = 0)

# Highlight maximum deviation
lines(c(x[Index], x[Index]), c(0, KSS[Index]), lwd = 3)
abline(v = x[Index], lty = 2)
```

![](ks-test_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->
