...
if (generate_pdf) pdf("AIC-Test.pdf", width = 6, height = 4)
p <- seq(1, 15, by = 1)
plot(p, qchisq(0.95, df = p), type = "l",
     ylim = c(1, 30), ylab = "Penalization", xlab = "p")
lines(p, 2 * p, col = "darkblue")
lines(p, log(100) * p, col = "darkgreen")
text(4, 25, "BIC", col = "darkgreen")
text(11, 25, "AIC", col = "darkblue")
text(12, 18, "Test")
```

![](AIC-und-TESTs_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 2: Probability of rejecting H0 under AIC and BIC
if (generate_pdf) pdf("AIC-Test-2.pdf", width = 6, height = 4)
plot(p, 1 - pchisq(2 * p, df = p), type = "l",
     ylim = c(0, 0.15), 
     ylab = expression(P( ~ "''" ~ H[1] ~ "''" ~ "|" ~ H[0] )), 
     xlab = "p", col = "darkblue")
abline(h = 0.05)
lines(p, 1 - pchisq(log(100) * p, df = p), col = "darkgreen")
text(4, 0.01, "BIC", col = "darkgreen")
text(4, 0.11, "AIC", col = "darkblue")
text(12, 0.06, "Test")
```

![](AIC-und-TESTs_files/figure-gfm/unnamed-chunk-1-2.png)<!-- -->

``` r
if (generate_pdf) dev.off()

# Plot 3: Probability comparison for large p
if (generate_pdf) pdf("AIC-Test-3.pdf", width = 6, height = 4)
p <- seq(20, 100, by = 1)
plot(p, 1 - pchisq(2 * p, df = p), type = "l",
     ylim = c(0, 0.001), 
     ylab = expression(P( ~ "''" ~ H[1] ~ "''" ~ "|" ~ H[0] )), 
...
![](AIC-und-TESTs_files/figure-gfm/unnamed-chunk-1-3.png)<!-- -->
